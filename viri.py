#Usage: python viri.py -n <val> -m <val> -s <seed_val> -o <outfile_name>
...
In this approach the requests are sorted in increasing order
where as vGPU profiles are sorted in increasing order of their vGPU 
memory or decreasing order of avaialble vGPU numbers.

# Here we are sorting the n_jk[][] in decreasing order and
#     correspinding a_jk[][] in increasing order of memory


Then first fit is applied for placing the requests in the vGPUs.
"""
#This doesn't returns the optimal  solution
...
"""
%%The basic algorihm:
>>Inputs: m_i[], n_jk[][],t_j[], a_jk[][]
>>Intermediate Data Structures: GPU_Profile_Set[] :  

1. Sort the requests(m_i[]) in decreasing order of their memory requests
2. Select the request 'i' from the list m_i:
        3. For each GPU j in the list of G_Set  :
	       4. if GPU_j[] is set &&  n_jk[j][GPU_prof_j[j]]>0 && a_jk[j][GPU_prof_j[j]]>=m_i[i]:
 			5. Place m_i on GPU_j
